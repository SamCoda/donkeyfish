...
import { Flame, Clock, TrendingUp, AlertCircle, ChevronUp, ChevronDown, DollarSign, Coins } from "lucide-react"
...
export function PresaleSection() {
  ...
  const [soldAmount, setSoldAmount] = useState(75000) // Example amount for demonstration
  ...
  useEffect(() => {
    const now = new Date()
    const targetDate = now < presaleStart ? presaleStart : now < presaleEnd ? presaleEnd : null
  ...
  useEffect(() => {
    const salesTimer = setInterval(() => {
      setSoldAmount((prev) => {
        ...
        // Update current stage based on sold amount
        if (newAmount <= 100000) {
          setCurrentStage(1)
        } else if (newAmount <= 200000) {
          setCurrentStage(2)
        ...
        return Math.min(newAmount, 1000000)
      })
    }, 5000)
    ...
    <section className="py-16 relative overflow-hidden bg-black">
      <div className="container mx-auto px-4">
        <div className="text-center mb-8">
          ...
          <h2 className="text-4xl md:text-5xl font-extrabold gold-text mb-4">Butanol Coin Presale</h2>
          <p className="text-white/80 max-w-2xl mx-auto">
            Join the exclusive presale of $BUTA tokens and be among the first to own the Toxic Frog King of Crypto!
          </p>
        </div>
          ...
          <div className="glass-card rounded-xl overflow-hidden border border-neon-green/30 shadow-xl">
            ...
            <div className="bg-gradient-to-r from-deep-green to-swamp-green p-6 border-b border-neon-green/30">
              <div className="flex flex-col md:flex-row justify-between items-center">
                <div>
                  <h3 className="text-2xl font-bold neon-text">$BUTA Token Presale</h3>
                  <p className="text-white/80">June 1 - June 10, 2025</p>
                </div>
                <div className="mt-4 md:mt-0">
                  <div className="bg-black/50 rounded-lg p-3 text-center">
                    <div className="text-xs text-white/70">{getCountdownLabel()}</div>
                    <div className="flex gap-2 mt-1">
                      <div className="bg-black/70 px-2 py-1 rounded">
                        <span className="text-xl font-bold neon-text">{timeLeft.days}</span>
                        <span className="text-xs block">Days</span>
                      </div>
                      <div className="bg-black/70 px-2 py-1 rounded">
                        <span className="text-xl font-bold neon-text">{timeLeft.hours}</span>
                        <span className="text-xs block">Hours</span>
                      </div>
                      <div className="bg-black/70 px-2 py-1 rounded">
                        <span className="text-xl font-bold neon-text">{timeLeft.minutes}</span>
                        <span className="text-xs block">Mins</span>
                      </div>
                      <div className="bg-black/70 px-2 py-1 rounded">
                        <span className="text-xl font-bold neon-text">{timeLeft.seconds}</span>
            ...
            <div className="p-6 border-b border-neon-green/30">
              <div className="flex flex-col md:flex-row justify-between items-center gap-4">
                ...
                <div>
                  ...
                  <div className="text-3xl font-bold neon-text">{currentStage} / 4</div>
                </div>
                <div>
                ...
                <div>
                  <Link href="#" className="toxic-button">
                    Buy $BUTA Now
                  </Link>
                </div>
              </div>
            ...
            <div className="p-6">
              <div className="mb-6">
                <div className="flex justify-between text-sm mb-1">
                  ...
                  <span className="text-neon-green">{getTotalProgress().toFixed(1)}%</span>
                </div>
              ...
              <div className="mb-4">
                <div className="flex justify-between text-sm mb-1">
                  ...
                  <span className="text-neon-green">{getStageProgress().toFixed(1)}%</span>
                </div>
              ...
              >
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="glass p-4 rounded-lg">
                    <h4 className="text-lg font-bold neon-text mb-3">Presale Stages</h4>
                    <ul className="space-y-3">
                      <li className="flex items-center gap-2">
                        <div
                          className={`w-3 h-3 rounded-full ${currentStage === 1 ? "bg-toxic-pink animate-pulse" : "bg-white/50"}`}
                        ></div>
                        <span className={currentStage === 1 ? "text-white" : "text-white/70"}>
                          Stage 1: 1 BUTA = $0.01 (0-100,000 BUTA)
                        </span>
                      </li>
                      <li className="flex items-center gap-2">
                        <div
                          className={`w-3 h-3 rounded-full ${currentStage === 2 ? "bg-toxic-pink animate-pulse" : "bg-white/50"}`}
                        ></div>
                        <span className={currentStage === 2 ? "text-white" : "text-white/70"}>
                          Stage 2: 1 BUTA = $0.02 (100,001-200,000 BUTA)
                        </span>
                      </li>
                      <li className="flex items-center gap-2">
                        <div
                          className={`w-3 h-3 rounded-full ${currentStage === 3 ? "bg-toxic-pink animate-pulse" : "bg-white/50"}`}
                        ></div>
                        <span className={currentStage === 3 ? "text-white" : "text-white/70"}>
                          Stage 3: 1 BUTA = $0.05 (200,001-500,000 BUTA)
                        </span>
                      </li>
                      <li className="flex items-center gap-2">
                        <div
                          className={`w-3 h-3 rounded-full ${currentStage === 4 ? "bg-toxic-pink animate-pulse" : "bg-white/50"}`}
                        ></div>
                        <span className={currentStage === 4 ? "text-white" : "text-white/70"}>
                  ...
                  <div className="glass p-4 rounded-lg">
                    <h4 className="text-lg font-bold neon-text mb-3">How to Participate</h4>
                    <ol className="space-y-3 list-decimal list-inside text-white/80">
                      <li>Connect your Solana wallet (Phantom, Solflare, etc.)</li>
                      <li>Enter the amount of $BUTA tokens you want to purchase</li>
                    ...
                    <div className="mt-4 flex items-center gap-2 text-toxic-pink">
                      <AlertCircle size={16} />
                      <span className="text-sm">Price increases as tokens are sold!</span>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <div className="mt-12 grid grid-cols-1 md:grid-cols-3 gap-6 max-w-4xl mx-auto">
          <div className="glass p-5 rounded-xl text-center transform hover:scale-105 transition-transform">
            ...
            <p className="text-white/80 text-sm">
              Be among the first to own $BUTA tokens before they hit exchanges at higher prices
            </p>
          </div>
          ...
          <div className="glass p-5 rounded-xl text-center transform hover:scale-105 transition-transform">
            ...
            <p className="text-white/80 text-sm">
              Presale participants receive exclusive NFT rewards and early governance rights
            </p>
          </div>
          ...
          <div className="glass p-5 rounded-xl text-center transform hover:scale-105 transition-transform">
            ...
            <p className="text-white/80 text-sm">
              Only 10 days to participate in this exclusive opportunity before public launch
            </p>
          </div>
        </div>
      </div>
